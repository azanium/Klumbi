...
public class UINavigationController : MonoBehaviour 
{
	...
	/// <summary>
	/// The popup controllers.
	/// </summary>
	public System.Collections.Generic.List<UIPopupViewController> popupControllers = new List<UIPopupViewController>();
	...
	public UIPopupViewController currentPopup;
    ...
    public void Start()
    {
        ...
        {
			if (view is UIPopupViewController)
			{
			...
			string controllerPath = view.controllerPath;
			if (string.IsNullOrEmpty(controllerPath))
			{
				controllerPath = string.Format("/{0}", view.GetType().ToString());
			}
		...
		UIPopupViewController[] popups = Resources.FindObjectsOfTypeAll(typeof(UIPopupViewController)) as UIPopupViewController[];

		foreach (UIPopupViewController popup in popups)
		{
			if (popupControllers.Contains(popup) == false)
			{
				popupControllers.Add(popup);
			}

			popup.gameObject.SetActive(false);

			string controllerPath = popup.controllerPath;
			if (string.IsNullOrEmpty(controllerPath))
			{
				controllerPath = string.Format("/{0}", popup.GetType().ToString());
			}
			int index = controllerPath.ToLower().IndexOf("controller");
			if (index >= 0)
			{
			...
			popup.controllerPath = controllerPath;
			if (mapControllerPath.ContainsKey(controllerPath) == false)
			{
				mapControllerPath.Add(controllerPath, popup);
			}
		}
	...
	private void ProcessControllerPath(ref UIViewController view)
	{
	...
	public string GetControllerPathFromParameters(Dictionary<string, string> parameters)
	{
		string path = "";
            ...
            if (mapControllerPath.ContainsKey(path))
            {
                ...
                if (view != null)
                {
                    ...
                    if (navData != null)
                    {
						...
						Debug.LogWarning("saving param: " + navData.parameter);
                    }
                }
            }
			else
	...
	public static void Popup(string controllerPath, float duration)
	{
		Popup(controllerPath, duration, null, null);
	}
    ...
    private void GetNavigationAnimation(UIViewController controller, bool isPush, out AnimationClip showAnimClip, out AnimationClip hideAnimClip)
    {
		...
		//Debug.Log("Show: " + showAnimClip.name);
		//Debug.Log("Hide: " + hideAnimClip.name);
    }
