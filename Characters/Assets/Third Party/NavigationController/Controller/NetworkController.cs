...
public class NetworkController : MonoBehaviour
{
	...
	public BoxCollider waitingCollider;
    ...
    {
        _ready = true;
		if (GetComponent<UIStretch>() != null)
		{
			GetComponent<UIStretch>().enabled = true;
		}
    }
	...
	private void ShowWaiting(bool state)
	{
		if (waitingCollider != null)
		{
			waitingCollider.enabled = state;
		}
		if (waitingSprite != null)
		{
			waitingSprite.enabled = state;
		}
	}
    ...
    public static void DownloadFromUrl(string path, Dictionary<string, object> postData, DownloadCallback onFinished)
    {
        ...
        {
			Debug.Log("DownloadFromUrl: " + path);
